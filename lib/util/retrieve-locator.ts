...
export function extractLocator(afterKey: string): FullSuffix {
  ...
  // Now before entering into the gruesome logic of extracting a correct locator, let's try
  // the simplest case: No locator provided. In this simplest of all cases, the first
  // character of the afterKey string will be a digit.
  ...
  retObject.suffix = afterKey.replace(locator[0], '') // Remove locator from the rest of the suffix
    }
  ...
  // If we've landed here, we have a locator label explicitly given. And therefore, the problems start.
  // How do we extract a locator label that might be given in all possible languages? Easy: We'll
  // simply do this by only supporting English. After all, each citation will be parsed internally
  // into citeproc, and for that matter the label and locator will be translated in final output. We
  // don't need to care so much about it now.
  ...
  // There is only one single problem: The locator may be divided into multiple parts,
  // e.g. 33-35, 149, 151. So we have to do multiple execs on the remaining suffix and
  // split the afterKey variable on the last found index.
  let splitIndex = 0
  let locatorRE = /(\d+(?:-\d+)?)/g
